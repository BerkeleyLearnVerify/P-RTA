...
const int n_lines = 4;
const char* screen[n_lines] = {"Robot: ", "Motion Planner: ", "Motion Primitives: ", "Battery: "};
...
PRT_VALUE* P_Init_IMPL(PRT_MACHINEINST* context, PRT_VALUE*** argRefs) {
  ...
  FILE* fp = fopen("../supervisor/scene", "r");
  char buffer[1024];
  int temp = 0;
...
PRT_VALUE* P_IsInTrajectory_IMPL(PRT_MACHINEINST* context, PRT_VALUE*** argRefs) {
  ...
  //printf("%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", x_0, z_0, x_1, z_1, x_position, z_position, angle2Goal/M_PI*180, orientation/M_PI*180, theta/M_PI*180, trajectoryDeviationDistance, trajectoryDeviation);
  if (trajectoryDeviation >= trajectoryDeviationThreshold) {
    return PrtMkBoolValue((PRT_BOOLEAN)false);
  }
  //printf("IN TRAJECTORY\n");
